...
namespace App_ConsultaStocks.Vistas
{
    ...
    public partial class Pedido_Picking_Item : ContentPage
    {
        int v_Id_CD = 0;
        string v_Id_Alm = "";
        ...
        private async Task<bool> MostrarSugerenciasPick(string message, List<SugerenciaPick> sugerencias)
        {
            ...
            var opciones = sugerencias
                .Select(s => $"{s.Ubicacion} | {s.Lote} | {s.Stock}")
                .ToArray();
        ...
        public Pedido_Picking_Item(Model_Pedido_Picking _Model_PedidoPicking)
        {
            ...
            v_TipoPicking = _Model_PedidoPicking.TIPO_PICKING;

            if (_Model_PedidoPicking.PICKING_MANUAL == 0)
            {
                ...
                if (v_TipoPicking == 2)
                {
                    pedido_Picking_SelPedidosxRutaItemOrden(v_Id_CD, v_IdPicking, int.Parse(lbl_Orden.Text));
                }
            ...
            else
            {
                ...
                //Picking_SelDatosRutaxOrden(v_IdPicking, int.Parse(lbl_Orden.Text));
                lbl_IdArticulo.Text = _Model_PedidoPicking.ID_ARTICULO;
                lbl_ITEMPEDIDO.Text = _Model_PedidoPicking.ITEMPEDIDO.ToString();
                lbl_DescArticulo.Text = _Model_PedidoPicking.ART_DESCRIPCION;
                lbl_CantUND.Text = _Model_PedidoPicking.CANTIDAD_PEDIDO.ToString();

                lbl_AtendidoUND.Text = "";
                lbl_Stock.Text = "";

                txt_IdUbicacion.Focus();

                Picking_SelDatosAdicionalesxItem(v_IdPicking, v_Id_Alm, int.Parse(lbl_ITEMPEDIDO.Text), lbl_IdArticulo.Text, 0);
            }
        }
        ...
        private void btnAnterior_Clicked(object sender, EventArgs e)
        {
            ...
            if (Chk_SoloPendientes.IsChecked == true)
            {
                ...
                lbl_IdUbicacion_Ruta.Text = "";
                lbl_ITEMPEDIDO.Text = "";
                lbl_IdArticulo.Text = "";
                lbl_DescArticulo.Text = "";
                lbl_CantUND.Text = "";
...

        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("alfa_usp_Pedido_Picking_SelDatosRutaxOrden", Conexion.conectar);
                ...
                if (Filas > 0)
                {
                    ...
                    lbl_IdLote.Text = Convert.ToString(dt.Rows[0][10]);
                ...
                if (Filas > 0)
                {
                    if (Filas == 1)
                    {
                        ...
                        if (txt_IdProducto.Text == lbl_IdArticulo.Text)
                        {
                            ...
                            VerifExistenciaIdArticuloUbicacion(txt_IdProducto.Text, v_Id_Alm, txt_IdUbicacion.Text, v_Id_CD, txt_IdLote.Text);
                            if (!string.IsNullOrWhiteSpace(lbl_IdLote.Text))
                            {
                                txt_IdLote.Focus();
                                stkLote.IsVisible = true;
        ...
        private async Task BuscarProductos_BarCode2(string CodBar, int Id_CD)
        {
            ...
            {
                // Si el helper no devolvió IdArticulo, tratar como error
            ...
            // Si no coincide con lo pedido, marcar rojo, avisar, limpiar y pedir reingreso
        ...
        private async void VerifExistenciaIdArticuloUbicacion(string ID_ARTICULO, string ID_ALMACEN, string ID_UBICACION, int Id_CD, string ID_LOTE = "")
        {
            Console.WriteLine($"[DEBUG] VerifExistenciaIdArticuloUbicacion called with ID_ARTICULO: {ID_ARTICULO}, ID_ALMACEN: {ID_ALMACEN}, ID_UBICACION: {ID_UBICACION}, Id_CD: {Id_CD}, ID_LOTE: {ID_LOTE}");
            try
            {
                DataTable dt = new DataTable();
                ...
                if (Id_CD == 1)
                {
                    ...
                    cmd.Parameters.Add("@ID_LOTE", SqlDbType.VarChar, 31).Value = ID_LOTE;
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    adapter.Fill(dt);
                    Conexion.Cerrar_WMS_LIM();
                }
        ...
        private async Task<bool> Picking_Det_InsDatos(
    int ID_PICKING,
    string ID_UBICACION,
    int ITEMPEDIDO,
    string ID_ARTICULO,
    ...
    string ID_LOTE_NEW
)
        {
            try
            {
                ...
                cmd.Parameters.Add("@ID_PICADOR", SqlDbType.Int).Value = 0; // Valor por defecto
                ...
                {
                    // OK: si existe mensaje informativo, mostrarlo (comentado para no mostrar alertas informativas)
        ...
        private void pedido_Picking_SelPedidosxRutaItemOrden(int ID_CD, int ID_PICKING, int ORDEN)
        {
            try
            {
                ...
                Conexion.Abrir();
                SqlCommand cmd = new SqlCommand("alfa_usp_pedido_Picking_SelPedidosxRutaItemOrden", Conexion.conectar);
                cmd.CommandType = CommandType.StoredProcedure;
